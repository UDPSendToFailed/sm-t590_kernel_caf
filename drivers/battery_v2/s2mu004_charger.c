...
static int s2mu004_charger_otg_control(struct s2mu004_charger_data *charger, bool enable)
{
	...
	pr_debug("%s S2MU004_CHG_STATUS2: 0x%x\n", __func__, chg_sts2);
	pr_debug("%s S2MU004_CHG_CTRL0: 0x%x\n", __func__, chg_ctrl0);
	pr_debug("%s 0xAE: 0x%x\n", __func__, temp);
...
static void s2mu004_analog_ivr_switch(struct s2mu004_charger_data *charger, int enable)
{
	...
	if (factory_mode || JIG_USB_ON) {
		pr_debug("%s: Factory Mode Skip Analog IVR Control\n", __func__);
		return;
	}
#if defined(CONFIG_BATTERY_SWELLING)
	...
	if (charger->charge_mode == SEC_BAT_CHG_MODE_CHARGING_OFF ||
		...
		(cable_type == SEC_BATTERY_CABLE_PREPARE_TA)) {
			pr_debug("[DEBUG]%s(%d): digital IVR \n", __func__, __LINE__);
			enable = 0;
	}
	s2mu004_read_reg(charger->i2c, 0xB3, &reg_data);
	pr_debug("%s : 0xB3 : 0x%x\n", __func__, reg_data);
	if (enable) {
		if (!(reg_data & 0x08)) {
			/* Enable Analog IVR */
			pr_debug("[DEBUG]%s: Enable Analog IVR\n", __func__);
			s2mu004_update_reg(charger->i2c, 0xB3, 0x1 << 3, 0x1 << 3);
		}
	} else {
		if (reg_data & 0x08) {
			/* Disable Analog IVR - Digital IVR enable*/
			pr_debug("[DEBUG]%s: Disable Analog IVR - Digital IVR enable\n", __func__);
...
static void s2mu004_set_regulation_voltage(struct s2mu004_charger_data *charger, int float_voltage)
{
	...
	if (factory_mode || JIG_USB_ON) {
		pr_debug("%s: Factory Mode Skip folat_voltage Control\n", __func__);
		return;
	}
...
static void s2mu004_set_input_current_limit(struct s2mu004_charger_data *charger, int charging_current)
{
	...
	if (factory_mode || JIG_USB_ON) {
		pr_debug("%s: Factory Mode Skip input_current Control\n", __func__);
		return;
	}
	mutex_lock(&charger->charger_mutex);
	if (is_wcin_type(charger->cable_type)) {
		pr_debug("[DEBUG]%s: Wireless current limit %d \n", __func__, charging_current);
		...
		pr_debug("[DEBUG]%s: Wired current limit %d \n", __func__, charging_current);
...
static void s2mu004_set_fast_charging_current(struct s2mu004_charger_data *charger, int charging_current)
{
	...
	if (factory_mode || JIG_USB_ON) {
		pr_debug("%s: Factory Mode Skip charging_current Control\n", __func__);
		return;
	}
	if (charging_current <= 100)
		data = 0x03;
...
static bool s2mu004_chg_init(struct s2mu004_charger_data *charger)
{
	...
	pr_debug("%s : S2MU004_CHG_CTRL17 : 0x%x\n", __func__, temp);
	...
	pr_debug("%s : S2MU004_CHG_CTRL13 : 0x%x\n", __func__, temp);
	...
	pr_debug("%s : S2MU004_CHG_CTRL12 : 0x%x\n", __func__, temp);
	/*
...
static void s2mu004_wdt_clear(struct s2mu004_charger_data *charger)
{
	...
	if (!(reg_data & 0x02)) {
		pr_debug("%s : WDT disabled BUT Clear requested!\n", __func__);
		pr_debug("%s : S2MU004_CHG_CTRL13 : 0x%x\n", __func__, reg_data);
		/* To prevent entering watchdog issue case we set WDT_CLR to not clear before enabling WDT */
...
static int s2mu004_get_charging_health(struct s2mu004_charger_data *charger)
{
	...
	pr_debug("[DEBUG] %s: S2MU004_CHG_STATUS1 0x%x\n", __func__, ret);
	...
	pr_debug("[DEBUG] %s: S2MU004_CHG_STATUS0 0x%x\n", __func__, ret);
	if (is_wcin_type(charger->cable_type)) {
						...
						const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		pr_debug("[DEBUG] %s: charger->cable_type(%d)\n", __func__, charger->cable_type);
	...
	case POWER_SUPPLY_PROP_CHARGE_UNO_CONTROL:
		{
			...
			pr_debug("%s: called charger uno control : %s\n", __func__,
				val->intval > 0 ? "ON" : "OFF");

			if (factory_mode) {
				u8 reg_val;

				pr_debug("%s: Can`t be tx boost when factory mode.\n", __func__);
				...
				pr_debug("%s S2MU004_CHG_CTRL18: 0x%x\n", __func__, reg_val);
				break;
			}
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		...
		if (val->intval && factory_mode) {
			pr_debug("%s: JIG Connection status: %d \n", __func__, val->intval);
			s2mu004_enable_charger_switch(charger, false);
		}
#endif
		break;
	...
	case POWER_SUPPLY_PROP_AUTHENTIC:
		if (val->intval) {
			pr_debug("%s: set Bypass mode by AT CMD for measuring sleep/off current\n", __func__);
			...
			pr_debug("%s additional setting start %d\n", __func__, __LINE__);
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		{
			...
			pr_debug("%s S2MU004_CHG_STATUS0: 0x%x\n", __func__, reg_data);
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_CURRENT_MEASURE:
			if (val->intval) {
				pr_debug("%s: set Bypass mode by keystring for measuring consumption current\n", __func__);
				/*
...
static irqreturn_t s2mu004_otg_fault_isr(int irq, void *data)
{
	...
	if ((val & OTG_STATUS_MASK) == (0x3 << OTG_STATUS_SHIFT)) {
		...
		pr_debug("%s: OTG OVERCURRENT LIMIT!!\n", __func__);
#ifdef CONFIG_USB_HOST_NOTIFY
...
static irqreturn_t s2mu004_chg_isr(int irq, void *data)
{
	...
	pr_debug("[IRQ] %s , STATUS0 : %02x\n ", __func__, val);
#if EN_OVP_IRQ
	if ((val & CHGIN_STATUS_MASK) == (2 << CHGIN_STATUS_SHIFT))	{
		charger->ovp = true;
		pr_debug("%s: OVP triggered\n", __func__);
...
static irqreturn_t s2mu004_event_isr(int irq, void *data)
{
	...
	pr_debug("[IRQ] %s , STATUS0:0x%02x, STATUS1:0x%02x, STATUS2:0x%02x, STATUS3:0x%02x\n",
...
static void s2mu004_ivr_irq_work(struct work_struct *work)
{
	...
	while ((ivr_state & IVR_STATUS) && !is_nocharge_type(charger->cable_type)) {
		...
		pr_debug("%s: S2MU004_CHG_STATUS3:0x%02x\n", __func__, ivr_state);
		if (!(ivr_state & IVR_STATUS)) {
			pr_debug("%s: EXIT IVR WORK: check value (S2MU004_CHG_STATUS3:0x%02x, input current:%d)\n",
...
static int s2mu004_charger_parse_dt(struct device *dev,	struct s2mu004_charger_platform_data *pdata)
{
	...
	if (!np) {
		...
		pr_debug("%s : pdata->wcinokb : %d\n", __func__, pdata->wcinokb);
		if (pdata->wcinokb < 0) {
			pr_debug("%s : can't get wcinokb\n", __func__);
			pdata->wcinokb = 0;
		}
	}
	...
	if (!np) {
		...
		ret = of_property_read_u32(np, "battery,support_pogo",
			&pdata->support_pogo);
		if (ret) {
			pr_debug("%s : support_pogo is Empty\n", __func__);
...
static int s2mu004_charger_probe(struct platform_device *pdev)
{
	...
	pr_debug("%s:[BATT] S2MU004 Charger driver probe\n", __func__);
...
static void s2mu004_charger_shutdown(struct device *dev)
{
	...
	pr_debug("%s: S2MU004 Charger driver shutdown\n", __func__);
	if (!charger->i2c) {
...
static int __init s2mu004_charger_init(void)
{
	...
	pr_debug("%s: S2MU004 Charger Init\n", __func__);
